...
namespace Nestor.Tools.Algorithms
{
    ...
    public static class NIR
    {
        ...
        public static string CalcKey(string nir)
        {
            ...
            var dep = nir.Substring(5, 2);
            var newDep = "";
            if (dep == "2A")
                newDep = "19";
            else if (dep == "2B") newDep = "18";
            if (newDep != "") nir = nir.Left(5) + newDep + nir.Right(6);

            decimal nirAsDecimal;
            if (!decimal.TryParse(nir, out nirAsDecimal)) throw new InvalidNirException();
        ...
        public static bool Check(string nir, string nirKey)
        {
            ...
            if (nirKey.Length != _NIRKeyExactLength || !int.TryParse(nirKey, out unusedInt))
                throw
                    new StringLengthException(); // ArgumentException(string.Format(ExceptionMessage.StringWrongLength, _NIRKeyExactLength));
