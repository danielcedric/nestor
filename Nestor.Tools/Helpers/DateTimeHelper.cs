...
namespace Nestor.Tools.Helpers
{
    ...
    public static class DateTimeHelper
    {
        ...
        private static readonly Dictionary<int, Dictionary<DateTime, FrenchHolidayType>> frenchHolidays =
            new Dictionary<int, Dictionary<DateTime, FrenchHolidayType>>();
        ...
        {
            var index = -1;
            var dateRetour = DateTime.MinValue;
            foreach (var formatDate in LstFormatDate)
                if (DateTime.TryParseExact(date, formatDate, CultureInfo.InvariantCulture, DateTimeStyles.None,
                    out dateRetour))
                {
        ...
        {
            var index = -1;
            var heureRetour = DateTime.MinValue;
            foreach (var formatHeure in LstFormatHeure)
                if (DateTime.TryParseExact(heure, formatHeure, CultureInfo.InvariantCulture, DateTimeStyles.None,
                    out heureRetour))
                {
        ...
        {
            return
                DateTime.Parse(dateHeure); //ParseExact(dateHeure, "yyyy-MM-dd HH:mm:ss", CultureInfo.CurrentCulture);
        }
        ...
        {
            var delta = target - start.DayOfWeek;
            if (delta <= 0)
                delta = 7 + delta;
        ...
        public static Dictionary<DateTime, FrenchHolidayType> GetFrenchHolidays(int year)
        {
            ...
            frenchHolidays.Add(year, new Dictionary<DateTime, FrenchHolidayType>(11)
            {
                ...
                {
                    new DateTime(year, easterMonday.Month, easterMonday.Day), FrenchHolidayType.EasterMonday
                }, // Lundi de Pâques
                {
                    new DateTime(year, dayWhitMonday.Month, dayWhitMonday.Day), FrenchHolidayType.DayWhitDay
                }, // Lundi de Pentecôte
                {
                    new DateTime(year, ascensionDay.Month, ascensionDay.Day), FrenchHolidayType.AscensionDay
                } // Jeud de l'ascension
            });
        ...
        public static string HumanizeValidity(DateTime dFrom, DateTime dTo)
        {
            if (dFrom.Year == dTo.Year)
            {
                if (dFrom.Month == dTo.Month)
                    return string.Format(Sentences.HumanizeValiditySameYearSameMonth, dFrom,
                        dTo); // Du 01 au 31 mars 2016
                return string.Format(Sentences.HumanizeValiditySameYearDifferentMonth, dFrom,
                    dTo); // Du 10 mars au 10 avril 2016
            }

            return string.Format(Sentences.HumanizeValidityDifferentYear, dFrom,
                dTo); // Du 12 décembre 2015 au 12 janvier 2016
        }
        ...
        public static DateTime GetMondayOfWeek(DateTime date)
        {
            ...
            var offset = date.DayOfWeek != DayOfWeek.Sunday ? date.DayOfWeek - DayOfWeek.Monday : 6;
