...
{
    internal class EquationSolvingHelper
    {
        #region Properties
        ...
        private static IDictionary<string, int[]> Operators
        {
            get
            {
                ...
                operators.Add("+", new[] {0, (int) AssocSide.Left});
                operators.Add("-", new[] {0, (int) AssocSide.Left});
                operators.Add("*", new[] {5, (int) AssocSide.Left});
                operators.Add("/", new[] {5, (int) AssocSide.Left});
        ...
        ///     equation splitée terme par terme afin de résoudre une équation de type : (False * False) +
        ///     False * (True * True)
        /// </param>
        ...
        public static string[] TransformToRPN(string[] inputTokens)
        {
            ...
            foreach (var token in inputTokens)
                ...
                if (IsOperator(token))
                {
                    ...
                    while (stack.Count > 0 && IsOperator(stack.Peek()))
                    {
                        ...
                        if (IsAssociative(token, AssocSide.Left) && ComparePrecedence(token, stack.Peek()) <= 0 ||
                            IsAssociative(token, AssocSide.Right) && ComparePrecedence(token, stack.Peek()) < 0)
                        {
                ...
                else if (token.Equals(")"))
                {
                    ...
                    while (stack.Count > 0 && !stack.Peek().Equals("(")) outResult.Add(stack.Pop());
            ...
            while (stack.Count > 0) outResult.Add(stack.Pop());
        ...
        {
            var ouput = TransformToRPN(input.Split(' '));
        ...
        private static double RPNToDouble(string[] tokens)
        {
            ...
            foreach (var token in tokens)
                ...
                {
                    var d2 = double.Parse(stack.Pop());
                    var d1 = double.Parse(stack.Pop());

                    var result = token.CompareTo("+") == 0 ? d1 + d2 :
        ...
        private static bool RPNToBool(string[] tokens)
        {
            ...
            foreach (var token in tokens)
                ...
                {
                    var b2 = bool.Parse(stack.Pop());
                    var b1 = bool.Parse(stack.Pop());

                    var result = token.CompareTo("*") == 0 ? b1 && b2 : b1 || b2;
                    stack.Push(result.ToString());
                }
        ...
        {
            if (!IsOperator(token)) throw new ArgumentException(string.Format("Jeton invalide: {0}", token));
